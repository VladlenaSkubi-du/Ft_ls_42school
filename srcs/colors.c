...
/*   Updated: 2019/06/30 18:47:28 by jcorwin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void        color_regular_stackfiles(t_file *file)
{
    ...
    if ((file->mode[3] == 'x' || file->mode[6] == 'x'
        ...
        || file->mode[6] == 'S')))
        ft_strcpy(file->color, "\033[31;40m");
    else
...
void        color_stackfiles(t_file *file)
{
    ...
    if (file->type == 'l') //но только название самой ссылки
        ft_strcpy(file->color, "\033[35;40m");
    else if (file->type == 'c')
        ft_strcpy(file->color, "\033[34;43m");
    else if (file->type == 'b')
        ft_strcpy(file->color, "\033[34;46m");
    else if (file->type == 's')
        ft_strcpy(file->color, "\033[32;40m");
    else if (file->type == 'p')
        ft_strcpy(file->color, "\033[33;40m");
    else if (file->type == 'd')
    {
        if (file->mode[8] == 'w' &&
            ...
            ft_strcpy(file->color, "\033[96;40m");
    }
    else if (file->type == '-')
